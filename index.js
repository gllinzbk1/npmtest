...
export function useAnalyzeRouteIsListOrContent(route) {
  ...
  if (newRP[2] != undefined) {
    ...
    if(element=='frameset') return 2;
    if ((pagenum != -1)) {
      //是page,就是列表
      return 1;
    } else {
      const Two = element.match(/--/g) //列表搜索特有的标签，返回索引位置
      const One = element.match(/-/g) //分页，内容页特有的标签，有些标签 值有-,返回数组列表或undefined
...
export function useChangeTagsHref(productStore, routeResult) {
  ...
  let queryurl = '';
  let allTagsResult = {}
  if (typeof productStore?.subCategory ==undefined) return allTagsResult;
  //--------------子类------------
  if (productStore.subCategory) {
    for (const key in productStore.subCategory) {
      if (Object.hasOwnProperty.call(productStore.subCategory, key)) {
        if(!isObject(productStore.subCategory[key]['href'])){
    ...
    console.log('productStore.subCategory', productStore.subCategory)
  } else {
  ...
  if (routeResult[1] == null || routeResult[1] == undefined) {
    ...
    console.log('route.subCategoriesdx', subCategories)
    productStore.subCategory['all']['checked'] = true
   // productStore.subCategory['all']['checked'] = true
  } else {
    console.log(22222,routeResult)
  ...
  if (routeResult[2] == 'quick') {
    ...
    productStore.subCategory[subCategories]['checked'] = false  
    productStore.subCategory['quick']['checked'] = true  
  }else{
    productStore.quick_buyActive = ''
  }
  
 console.log('route.subCategories', subCategories)
  ...
  if (routeResult[2] ) {//&& routeResult[2]!=undefined
    ...
    if (searchkey != undefined) {
      searchkey.forEach(searchElement => {
        ...
        const tagkey = productStore.subCategory['all']['kvTags'][searchElement];
